...
create schema if not exists impl_variable_expression;
create stage if not exists impl_variable_expression.udf_stage; 

-- Run manually: `uv run stage_python.py live schemas/impl_variable_expression/functions/`


-- Deploying: schemas\impl_weight\procedures\rim_weighting_calculator.sql
create or replace procedure rim_weighting_calculator(
    quota_cells variant,
    rim_dimensions variant,
...
-- Deploying: schemas\impl_variable_expression\functions\_evaluate_data_wave_variable_for_response.sql
...
-- Deploying: schemas\impl_variable_expression\functions\_evaluate_survey_id_variable_for_response.sql
...
-- Deploying: schemas\impl_variable_expression\functions\_parse_python_expression.sql
...
-- Deploying: schemas\impl_variable_expression\tables\derived_variable_answers.sql
-- Non-transient table for derived variable answers
...
-- Deploying: schemas\impl_variable_expression\tables\_derived_variable_calculation_history.sql
...
-- Deploying: schemas\savanta_internal__utils\procedures\clone_full.sql
...
-- Deploying: schemas\impl_sub_product\tables\variable_configuration_dependencies.sql
...
create or alter transient table impl_weight._hardcoded_target_weights (
    response_set_id number(38, 0) not null,
    ...
    select
        ...
        case
            when answer_entity_type_identifier is not null
                then array_append(array_remove(pe.parse_result:"result_entity_identifiers"::array, answer_entity_type_identifier::variant), answer_entity_type_identifier)
            else pe.parse_result:"result_entity_identifiers"::array
        end as entity_identifiers,
        pe.parse_result:"dependency_variable_identifiers"::array as dependency_variable_identifiers,
        -- Map entity identifiers to asked_entity_type columns (first 3 in alphabetical order)
        case
            when array_size(pe.parse_result:"result_entity_identifiers") > 0
                then pe.parse_result:"result_entity_identifiers"[0]::string
            else null
        end as asked_entity_type_identifier_1,
        case
            when array_size(pe.parse_result:"result_entity_identifiers") > 1
                then pe.parse_result:"result_entity_identifiers"[1]::string
            else null
        end as asked_entity_type_identifier_2,
        case
            when array_size(pe.parse_result:"result_entity_identifiers") > 2
                then pe.parse_result:"result_entity_identifiers"[2]::string
            else null
        end as asked_entity_type_identifier_3,
...
-- Deploying: schemas\impl_response_set\tables\variable_dependencies.sql
...
(
    ...
    from impl_variable_expression.derived_variables dv,
        lateral flatten(input => dv.dependency_variable_identifiers) as dep_identifiers
);
...
-- Deploying: schemas\impl_variable_expression\views\_uncached_response_counts.sql
...
(
    with distinct_canonical_choice_sets as (
        ...
        select distinct
            ...
            first_value(canonical_survey_id) over (partition by response_set_id, canonical_choice_set_id order by canonical_survey_id) as canonical_survey_id
...
-- Deploying: schemas\impl_response_set_unconfigured\tables\_alternative_choice_set_to_default_entity_identifier_mappings.sql
    ...
    select
        ...
        case
            ...
            when
                ...
                or row_number() over (partition by jv.response_set_id, jv.question_var_code order by jv.canonical_survey_id asc) > 1
                then jv.question_var_code || '_' || jv.canonical_survey_id
...
(
    ...
    select
        ...
        false as question_is_multiple_choice, -- derived variables are not multiple choice questions
        object_construct(
        ...
        array_construct_compact(dv.asked_entity_type_identifier_1, dv.asked_entity_type_identifier_2, dv.asked_entity_type_identifier_3) as asked_entity_type_identifiers
...
-- Deploying: schemas\impl_variable_expression\tables\_variable_entity_types.sql
...
-- Deploying: schemas\impl_variable_expression\tables\_derived_variable_dependency_shapes.sql
...
-- Deploying: schemas\impl_variable_expression\tables\_entity_instances_arrays.sql
...
-- Deploying: schemas\impl_variable_expression\tables\_entity_instance_ids_2.sql
create or replace transient dynamic table impl_variable_expression._entity_instance_ids_2
target_lag = 'DOWNSTREAM' refresh_mode = incremental initialize = on_create warehouse = warehouse_xsmall
as
(
    select response_set_id, entity_type_identifier, entity_instance_id
    from impl_response_set.entity_instances
);


-- Deploying: schemas\impl_variable_expression\tables\_entity_instance_ids_3.sql
create or replace transient dynamic table impl_variable_expression._entity_instance_ids_3
target_lag = 'DOWNSTREAM' refresh_mode = incremental initialize = on_create warehouse = warehouse_xsmall
as
(
    select response_set_id, entity_type_identifier, entity_instance_id
...
-- Deploying: schemas\impl_variable_expression\tables\_entity_instance_ids_4.sql
create or replace transient dynamic table impl_variable_expression._entity_instance_ids_4
target_lag = 'DOWNSTREAM' refresh_mode = incremental initialize = on_create warehouse = warehouse_xsmall
as
(
    select response_set_id, entity_type_identifier, entity_instance_id
...
-- Deploying: schemas\impl_variable_expression\streams\_derived_variable_dependency_shapes_stream.sql
...
-- Deploying: schemas\impl_variable_expression\tables\_variable_entity_instance_arrays.sql
...
create or replace transient dynamic table impl_response_set.variable_answers (
    ...
    asked_entity_id_3 integer,
    answer_value integer
) cluster by (response_set_id, variable_identifier, response_id) -- When evaluating python expressions this halves the time taken and significantly reduces partitions scanned
target_lag = 'DOWNSTREAM' refresh_mode = incremental initialize = on_create warehouse = warehouse_xsmall
as
(
...
-- Deploying: schemas\impl_variable_expression\tables\_derived_variables_with_shapes.sql
...
-- State table to track when variable answers are available until for derived variable computation
-- Uses actual survey response completion dates
create or replace transient dynamic table impl_variable_expression._variable_availability_state (
    response_set_id integer,
    variable_identifier varchar(256),
    latest_answer_date timestamp_ntz
) target_lag = 'DOWNSTREAM' refresh_mode = incremental initialize = on_create warehouse = warehouse_xsmall
as
(
    -- Track the latest answer date for each variable as a proxy for when all data is available
    -- This ensures derived variables wait until their dependencies have been fully computed
    with variable_answer_dates as (
        select
            va.response_set_id,
            va.variable_identifier,
            -- Use the creation/update timestamp if available, otherwise current time
            -- This tracks when the answer data was last modified
            max(r.survey_completed) as latest_answer_date
        from impl_response_set.variable_answers va
        inner join impl_response_set.responses r on va.response_id = r.response_id
        group by va.response_set_id, va.variable_identifier
    )

    select
        vad.response_set_id,
        vad.variable_identifier,
        vad.latest_answer_date
    from variable_answer_dates vad
);


-- Deploying: schemas\impl_variable_expression\views\_variable_answer_arrays.sql
...
(
    select
        ...
        dv.variable_identifier
    from impl_variable_expression.derived_variables dv
    inner join impl_response_set.variable_dependencies vd
        on dv.response_set_id = vd.response_set_id and dv.variable_identifier = vd.variable_identifier
    inner join impl_variable_expression._variable_availability_state vas
        on dv.response_set_id = vas.response_set_id and vd.depends_on_variable_identifier = vas.variable_identifier
    left join impl_variable_expression._variable_availability_state mvas
        on dv.response_set_id = mvas.response_set_id and vd.variable_identifier = mvas.variable_identifier
    left join impl_variable_expression._derived_variable_calculation_history dvch
        on dv.response_set_id = dvch.response_set_id and dv.variable_identifier = dvch.variable_identifier
    where dvch.response_set_id is null -- Not yet calculated
    group by dv.response_set_id, dv.variable_identifier, dv.dependency_variable_identifiers, mvas.latest_answer_date
    having
        count(*) = array_size(dv.dependency_variable_identifiers)
        and coalesce(mvas.latest_answer_date, '2000-01-01') < min(vas.latest_answer_date)
);
...
select
    ...
    dv.variable_identifier,
    vaa.response_id,
    object_agg(vaa.variable_identifier, vaa.answer_arrays) as answer_arrays_by_variable_identifier
from impl_variable_expression.derived_variables dv
inner join impl_variable_expression._derived_variable_dependency_mappings ddm
    on dv.response_set_id = ddm.response_set_id and dv.variable_identifier = ddm.variable_identifier
inner join impl_variable_expression._variable_answer_arrays vaa
    on
        dv.response_set_id = vaa.response_set_id and
        ddm.dependency_variable_identifier = vaa.variable_identifier
group by dv.response_set_id, dv.variable_identifier, vaa.response_id;
...
-- Deploying: schemas\impl_variable_expression\views\_uncached_data_wave_variable_answer_arrays.sql
...
-- Deploying: schemas\impl_variable_expression\views\_uncached_expression_variable_answer_arrays.sql
...
(
    ...
    inner join impl_variable_expression._dependency_answers da
        on dv.response_set_id = da.response_set_id and dv.variable_identifier = da.variable_identifier
    where dv.variable_type = 'expression'
);


-- Deploying: schemas\impl_variable_expression\views\_uncached_survey_id_variable_answer_arrays.sql
...
-- Deploying: schemas\impl_variable_expression\views\_uncached_derived_answers.sql
...
-- Deploying: schemas\impl_variable_expression\procedures\_update_derived_variable_answers.sql
...
declare
    ...
    calculation_time timestamp_ntz := current_timestamp();
    error_msg string;
begin
    ...
    let calculation_end_time := current_timestamp();
...
-- Deploying: schemas\impl_variable_expression\tasks\_incremental_update_derived_variable_answers.sql
...
as begin
    ...
    from impl_variable_expression._variable_calculation_status;
    ...
    let start_time timestamp_ltz := current_timestamp();
    for variable in variables_to_update do
        if (datediff(minute, start_time, current_timestamp()) >= 30) then
...
-- Deploying: schemas\impl_variable_expression\procedures\init.sql
...
call impl_variable_expression.init();
