...
using Mixpanel;
using static BrandVue.MixPanel.VueEventsDictionary;
...
namespace BrandVue.MixPanel
{
    ...
    public static class MixPanel
    {
        ...
        private static ILogger<MixPanelLogger> _logger = null!;
        ...
        public static async Task TrackAsync(TrackAsyncEventModel model)
        {
            ...
            if (_client is not null)
            {
                ...
                if (!result)
                {
                    _logger.LogWarning("Failed to track event {eventName}", model.EventName);
                }
            }
        }
